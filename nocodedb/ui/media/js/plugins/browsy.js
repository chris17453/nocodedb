(function ( $ ) {
	...
	<div class="row bg-light border border-left-0 border-right-0 border-top-0 border-secondary">
	   <div class="col bg-light font-weight-bold pl-3 p-1"><i class="fas fa-folder-open"></i> <span class="browsy-path"></span></div>
	</div>
	<div class="row browsy-row">
	   ...
	   <div class="col-7 p-0 browsy-row">
		  <table class="table browsy-table-fixed bg-light table-hover mb-0">
			 <thead>
				<tr>
				   <th class="col-8">Name</th>
				   <th class="col-2 text-right">Size</th>
				   <th class="col-2 text-right">Modified</th>
				</tr>
			 </thead>
			 <tbody class="browsy-file-container bg-white table-sm">
			 ...
			 <tfoot>
				<tr>
				   ...
				   <td class="col-2 text-right">Modified</td>
				</tr>
			 </tfoot>
		  </table>
	   </div>
	   <div class="col-2 p-0 bg-light browsy-row">
	   <div class="browsy-info-container">
		  ...
		  <div class="col mt-3 bg-light border border-left-0 border-right-0 border-top-0 border-secondary"></div>
		  ...
		  <div class="col mt-3 bg-light border border-left-0 border-right-0 border-top-0 border-secondary browsy-admin-line"></div>
	   </div>
	</div>
	</div>
</div>`
						...
						switch(l) {
							...
							case 2: 	if(l==2 && file_item.type==="file") { //no specials for files yet
											...
											$(this.element).find(".browsy-file-container").append(
												'<tr class="browsy-file" data-file="'+ei.name+'">'+
		...
		browser.prototype.load=function(file_path,filters,absolute=false) {
			...
			this.call_server("server/get_dir",function(jr) {
				self.path       =jr.path;
				self.files      =jr.files;
		...
		browser.prototype.call_server =function(method,success_func,err_func,da){
			...
			$.ajax({type: "POST", url: this.webServiceURL +"/" + method, data:JSON.stringify(this.data_object),
